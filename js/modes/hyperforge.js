...
const HyperforgeMode = {
    name: 'Hyperforge',
    group: null, time: 0,
    mainMesh: null, mainWire: null, basePositions: null, normals: null, vertexColors: null,
    innerMesh: null, innerWire: null, innerBasePos: null, innerNormals: null,
    attractorSystem: null, attractorPositions: null, attractorColors: null, attractorVelocities: [], maxAttractorParts: 8000,
    flowSystem: null, flowPositions: null, flowColors: null, flowVelocities: [], maxFlowParts: 5000,
    ...
    params: {
        ...
        outerSurface: {
            ...
            ], default: 'superformula', label: '🔮 Outer Surface'
        },
        outerDetail: { type: 'range', min: 10, max: 80, default: 40, step: 5, label: 'Resolution' },
        outerSize: { type: 'range', min: 5, max: 50, default: 22, step: 1, label: 'Size' },
        sfM: { type: 'range', min: 1, max: 20, default: 6, step: 0.5, label: '🔢 SF m (sym)' },
        sfN1: { type: 'range', min: 0.1, max: 10, default: 1, step: 0.1, label: 'SF n1' },
        sfN2: { type: 'range', min: 0.1, max: 10, default: 1, step: 0.1, label: 'SF n2' },
        sfN3: { type: 'range', min: 0.1, max: 10, default: 1, step: 0.1, label: 'SF n3' },
        sfAudioMap: { type: 'toggle', default: true, label: '🎵 Audio → SF' },
        // ── INNER SURFACE ──
        showInner: { type: 'toggle', default: true, label: '🔵 Show Inner' },
        innerSurface: { type: 'select', options: ['sphere', 'torusKnot', 'icosahedron', 'superformula', 'none'], default: 'icosahedron', label: 'Inner Surface' },
        innerScale: { type: 'range', min: 0.1, max: 0.9, default: 0.4, step: 0.05, label: 'Inner Scale' },
        ...
        displaceMode: {
            ...
            ], default: 'fourier', label: '🌊 Displace'
        },
        displaceAmt: { type: 'range', min: 0, max: 30, default: 8, step: 0.5, label: 'Displace Amt' },
        displaceSpeed: { type: 'range', min: 0.1, max: 5, default: 1.5, step: 0.1, label: 'Displace Speed' },
        symmetryAxis: { type: 'select', options: ['off', 'x', 'y', 'z', 'xy', 'xz', 'yz', 'xyz'], default: 'off', label: '🔀 Symmetry' },
        gravWellCount: { type: 'range', min: 1, max: 5, default: 2, step: 1, label: '🕳️ Grav Wells' },
        // ── ATTRACTOR ──
        attractorType: { type: 'select', options: ['lorenz', 'rossler', 'aizawa', 'thomas', 'halvorsen', 'chen', 'dadras', 'sprott', 'none'], default: 'lorenz', label: '🌀 Attractor' },
        secondAttractor: { type: 'select', options: ['none', 'lorenz', 'rossler', 'aizawa', 'thomas', 'halvorsen', 'chen', 'dadras', 'sprott'], default: 'none', label: '🔗 2nd Attractor' },
        attractorBlend: { type: 'range', min: 0, max: 1, default: 0.5, step: 0.05, label: 'Blend' },
        attractorCount: { type: 'range', min: 500, max: 10000, default: 4000, step: 500, label: 'Attractor Pts' },
        ...
        attractorAudioLink: { type: 'select', options: ['bass', 'mid', 'treble', 'rms', 'sub'], default: 'bass', label: '🔊 Audio Link' },
        particleJitter: { type: 'range', min: 0, max: 5, default: 0, step: 0.1, label: '✨ Jitter' },
        showTrails: { type: 'toggle', default: true, label: '📈 Trails' },
        trailColorMode: { type: 'select', options: ['velocity', 'time', 'distance', 'palette'], default: 'velocity', label: '🌈 Trail Color' },
        // ── FLOW ──
        flowEnabled: { type: 'toggle', default: true, label: '💫 Flow' },
        flowCount: { type: 'range', min: 500, max: 8000, default: 3000, step: 500, label: 'Flow Count' },
        ...
        // ── RENDERING ──
        wireOpacity: { type: 'range', min: 0, max: 1, default: 0.7, step: 0.05, label: 'Wire Opacity' },
        solidOpacity: { type: 'range', min: 0, max: 0.6, default: 0.2, step: 0.05, label: 'Solid Opacity' },
        colorMode: {
            type: 'select', options: [
        ...
        pointGlow: { type: 'range', min: 0.5, max: 6, default: 2.5, step: 0.5, label: '✨ Glow' },
        // ── BEHAVIOUR ──
        rotSpeed: { type: 'range', min: 0, max: 3, default: 0.4, step: 0.05, label: 'Rotation' },
    ...
    getOuterGeo(shape, seg, size, m, n1, n2, n3) {
        switch (shape) {
            ...
            case 'kleinBottle': return this._grid(seg, (u, v) => {
                ...
                let x, y; if (a < Math.PI) { x = (2.5 - 1.5 * Math.cos(a)) * s2 * Math.cos(b) * 6; y = (2.5 - 1.5 * Math.cos(a)) * s2 * Math.sin(b) * 6; }
                else { x = (-2 + (2 + Math.cos(b)) * Math.cos(a)) * s2 * 6; y = (2 + Math.cos(b)) * Math.sin(a) * s2 * 6; }
                return [x, y, -Math.sin(b) * (2.5 - 1.5 * Math.cos(a)) * s2 * 3];
            });
            ...
            case 'diniSurface': return this._grid(seg, (u, v) => {
                const a2 = u * Math.PI * 4, b2 = v * 2 + 0.01, s2 = size * 0.5;
                return [s2 * Math.cos(a2) * Math.sin(b2), s2 * (Math.cos(b2) + Math.log(Math.tan(b2 / 2)) + 0.2 * a2 * 0.1), s2 * Math.sin(a2) * Math.sin(b2)];
            });
            case 'enneperSurface': return this._grid(seg, (u, v) => {
                const a2 = (u - 0.5) * 3, b2 = (v - 0.5) * 3, s2 = size * 0.3;
                return [s2 * (a2 - a2 * a2 * a2 / 3 + a2 * b2 * b2), s2 * (b2 - b2 * b2 * b2 / 3 + a2 * a2 * b2), s2 * (a2 * a2 - b2 * b2)];
            });
            case 'crossCap': return this._grid(seg, (u, v) => {
                const a2 = u * Math.PI, b2 = v * Math.PI * 2;
                return [size * Math.cos(a2) * Math.sin(2 * b2) * 0.5, size * Math.sin(a2) * Math.sin(2 * b2) * 0.5, size * (Math.cos(b2) ** 2 - Math.cos(a2) ** 2 * Math.sin(b2) ** 2) * 0.5];
            });
            case 'boysSurface': return this._grid(seg, (u, v) => {
                const a2 = u * Math.PI, b2 = v * Math.PI * 2, s2 = size * 0.5;
                const sq2 = Math.SQRT2, ca = Math.cos(a2), sa = Math.sin(a2), cb = Math.cos(b2), sb = Math.sin(b2);
            ...
            case 'romanSurface': return this._grid(seg, (u, v) => {
                const a2 = u * Math.PI, b2 = v * Math.PI * 2, s2 = size * 0.5;
                return [s2 * Math.sin(2 * a2) * Math.cos(b2) ** 2, s2 * Math.sin(a2) * Math.sin(2 * b2) / 2, s2 * Math.cos(a2) * Math.sin(2 * b2) / 2];
            });
            case 'seiferSurface': return this._grid(seg, (u, v) => {
            ...
            case 'steinerian': return this._grid(seg, (u, v) => {
                const a2 = (u - 0.5) * 2, b2 = (v - 0.5) * 2, s2 = size * 0.8;
                return [s2 * a2 * b2, s2 * a2 * (1 - b2 * b2), s2 * b2 * (1 - a2 * a2)];
            });
            case 'trefoilKnot': return this._grid(seg, (u, v) => {
                const t2 = u * Math.PI * 2, w2 = (v - 0.5) * size * 0.12, r2 = Math.cos(3 * t2) + 2;
                return [(r2 * Math.cos(2 * t2) + w2 * Math.cos(2 * t2) * Math.cos(3 * t2)) * size * 0.25, (r2 * Math.sin(2 * t2) + w2 * Math.sin(2 * t2) * Math.cos(3 * t2)) * size * 0.25, (Math.sin(3 * t2) + w2 * Math.sin(3 * t2)) * size * 0.25];
            });
            case 'algebraicHorn': return this._grid(seg, (u, v) => {
                const a2 = u * Math.PI * 2, b2 = v * Math.PI * 2, s2 = size * 0.3;
                return [s2 * (2 + Math.cos(b2)) * Math.cos(a2) * (1 + u * 0.5), s2 * (2 + Math.cos(b2)) * Math.sin(a2) * (1 + u * 0.5), s2 * (Math.sin(b2) + u * 4)];
            });
    ...
    buildOuter(shape, seg, size, m, n1, n2, n3) {
        if (this.mainMesh) { this.group.remove(this.mainMesh); this.mainMesh.geometry.dispose(); this.mainMesh.material.dispose(); }
        if (this.mainWire) { this.group.remove(this.mainWire); this.mainWire.geometry.dispose(); this.mainWire.material.dispose(); }
    ...
    buildInner(shape, size) {
        if (this.innerMesh) { this.group.remove(this.innerMesh); this.innerMesh.geometry.dispose(); this.innerMesh.material.dispose(); }
        if (this.innerWire) { this.group.remove(this.innerWire); this.innerWire.geometry.dispose(); this.innerWire.material.dispose(); }
        this.innerMesh = null; this.innerWire = null;
        if (shape === 'none') { this.currentInner = 'none'; this.currentInnerSize = size; return; }
        let geo;
    ...
    initAttractor(count) {
        ...
        for (let i = 0; i < count; i++) {
            this.attractorPositions[i * 3] = (Math.random() - 0.5) * 2; this.attractorPositions[i * 3 + 1] = (Math.random() - 0.5) * 2; this.attractorPositions[i * 3 + 2] = (Math.random() - 0.5) * 2;
        ...
        this.attractorSystem = new THREE.Points(geo, new THREE.PointsMaterial({ size: 1.5, vertexColors: true, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true }));
    ...
    initFlow(count) {
        ...
        this.flowVelocities = [];
        for (let i = 0; i < count; i++) { const a = Math.random() * Math.PI * 2, r = 10 + Math.random() * 15; this.flowPositions[i * 3] = Math.cos(a) * r; this.flowPositions[i * 3 + 1] = (Math.random() - 0.5) * 20; this.flowPositions[i * 3 + 2] = Math.sin(a) * r; this.flowVelocities.push({ x: 0, y: 0, z: 0 }); }
        const geo = new THREE.BufferGeometry();
        ...
        this.flowSystem = new THREE.Points(geo, new THREE.PointsMaterial({ size: 1, vertexColors: true, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true }));
    ...
    initTrails() {
        ...
        this.trailLine = new THREE.Line(geo, new THREE.LineBasicMaterial({ color: 0xff44aa, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending }));
    ...
    update(audio, params, dt) {
        ...
        const bass = audio.smoothBands.bass || 0, sub = audio.smoothBands.sub || 0, mid = audio.smoothBands.mid || 0, treble = audio.smoothBands.treble || 0, rms = audio.rms || 0;

        // Rebuild check — throttled to max once per 200ms
        const shape = params.outerSurface || 'superformula', seg = Math.floor(params.outerDetail || 40), size = params.outerSize || 22;
        let m = params.sfM || 6, n1 = params.sfN1 || 1, n2 = params.sfN2 || 1, n3 = params.sfN3 || 1;
        if (params.sfAudioMap && shape === 'superformula') {
            // Smooth lerp SF params instead of discrete jumps
        ...
        if (shape !== this.currentShape) {
            ...
            this.lastSfParams = sfKey;
            this.lastRebuildTime = now;
        } else if (shape === 'superformula' && sfKey !== this.lastSfParams && (now - this.lastRebuildTime) > 200) {
            // Only rebuild when params changed AND throttle at 200ms
            this.buildOuter(shape, seg, size, m, n1, n2, n3);
            this.lastSfParams = sfKey;
            this.lastRebuildTime = now;
        }

        // Inner surface (fixed: track size changes)
        ...
        // Displacement
        this.displaceSurface(audio, params, dt);

        // Attractors
        this.updateAttractor(audio, params, dt);
        ...
        const rot = (params.rotSpeed || 0.4) * (1 + mid * react * 0.6);
        this.group.rotation.x += rot * 0.3 * dt; this.group.rotation.y += rot * dt; this.group.rotation.z += rot * 0.1 * dt;
        if (audio.bassBeat) {
            const maxBurst = audio.isHighEnergy ? 0.08 : 0.15;
            this.group.rotation.y += Math.min(maxBurst, audio.bassBeatIntensity * 0.2);
        }

        // Beat explode — controlled growth
        ...
        // Inner pulsing
        if (this.innerMesh) { const ip = 1 + (sub + bass) * 0.3; this.innerMesh.scale.setScalar((params.innerScale || 0.4) * ip); if (this.innerWire) this.innerWire.scale.setScalar((params.innerScale || 0.4) * ip); }
    },
    ...
    displaceSurface(audio, params, dt) {
        ...
        const col = this.mainMesh.geometry.attributes.color.array;
        ...
        for (let i = 0; i < count; i++) {
            ...
            let disp = 0;
            switch (mode) {
                case 'fourier': { const h1 = Math.sin(sx * 0.3 + this.time * speed) * freq; const h2 = Math.sin(sy * 0.5 + this.time * speed * 0.7) * (audio.smoothBands.mid || 0); const h3 = Math.sin(sz * 0.2 + this.time * speed * 1.3) * (audio.smoothBands.treble || 0); disp = (h1 + h2 + h3) * amt; break; }
                ...
                case 'gravitationalWell': {
                    ...
                    for (let w = 0; w < wells; w++) {
                        const wa = (w / wells) * Math.PI * 2 + this.time * 0.3;
                        const wx = Math.cos(wa) * 10, wz = Math.sin(wa) * 10;
                        const dx = sx - wx, dz = sz - wz, d2 = Math.sqrt(dx * dx + sy * sy + dz * dz) + 0.1;
                        totalForce += bass * 15 / (d2 * d2 + 1);
                    }
    ...
    updateAttractor(audio, params, dt) {
        ...
        for (let i = 0; i < this.maxAttractorParts; i++) {
            ...
            // Blend with second attractor
            if (type2 !== 'none') {
                const [dx2, dy2, dz2] = this.stepAttractor(type2, x, y, z, speed, audioMod);
                dx = dx * (1 - blend) + dx2 * blend; dy = dy * (1 - blend) + dy2 * blend; dz = dz * (1 - blend) + dz2 * blend;
            }
    ...
    updateFlow(audio, params, dt) {
        ...
        for (let i = 0; i < this.maxFlowParts; i++) {
            ...
            if (pattern === 'orbit') {
                v.x += Math.cos(angle) * (15 - dist) * 0.01 * speed - Math.sin(angle) * (0.5 + bass) * speed * 2;
                v.y += Math.sin(this.time + i * 0.01) * mid * speed;
                v.z += Math.sin(angle) * (15 - dist) * 0.01 * speed + Math.cos(angle) * (0.5 + bass) * speed * 2;
                ...
                v.x = Math.cos(ha) * speed * (1 + bass); v.z = Math.sin(ha) * speed * (1 + bass);
                v.y += Math.sin(this.time * 3 + i * 0.05) * speed * 0.5;
            } else if (pattern === 'chaos') {
                v.x += (Math.random() - 0.5) * speed * bass * 3; v.y += (Math.random() - 0.5) * speed * mid * 3; v.z += (Math.random() - 0.5) * speed * bass * 3;
            } else if (pattern === 'vortex') {
                const vDist = 15 - dist; v.x += -Math.sin(angle) * speed * 3 * (1 + bass) + Math.cos(angle) * vDist * 0.005;
                v.z += Math.cos(angle) * speed * 3 * (1 + bass) + Math.sin(angle) * vDist * 0.005;
    ...
    // Ring buffer trails (fixed from array splice)
    ...
    destroy(scene) {
        if (this.group) { this.group.traverse(c => { if (c.geometry) c.geometry.dispose(); if (c.material) c.material.dispose(); }); scene.remove(this.group); }
