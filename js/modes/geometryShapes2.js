...
// 25 shapes, 16 displacement modes, full tearout reactivity
// Ghost trails, particles, mirrors, morphing, symmetry
// ============================================================

const GeometryForgeMode2 = {
    name: 'Geometry Forge old',
    ...
    init(scene, camera) {
        ...
        this.morphing = false; this.morphTargetBase = null; this.morphProgress = 0;
    ...
    buildGeometry(shape, detail, size) {
        ...
        this.meshWire = new THREE.LineSegments(
            new THREE.WireframeGeometry(this.meshSolid.geometry),
            new THREE.LineBasicMaterial({ color: 0x8b5cf6, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending })
        );
        this.group.add(this.meshWire);

        this.meshInnerWire = new THREE.LineSegments(
            new THREE.WireframeGeometry(this.meshSolid.geometry),
            new THREE.LineBasicMaterial({ color: 0xff5cf6, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending })
        );
    ...
    update(audio, params, dt) {
        ...
        const progRamp  = (behavior && behavior.useSectionProgressRamp)
                          ? (0.5 + 0.5 * (audio.sectionProgress || 0)) : 1.0;

        // Section entry → auto-select displaceMode + phase snap
        if (audio.sectionChanged) {
            const modeMap = {
                drop:      'shatter', drop2:    'glitch',  climax:    'harmonics',
                breakdown: 'melt',    buildup:  'spike',   intro:     'noise',
                verse:     'frequency', fakeout: 'breathe', bridge:   'ripple',
                outro:     'breathe'
            };
            if (modeMap[audio.sectionType]) this._sectionDisplaceMode = modeMap[audio.sectionType];
        }

        // Fakeout: collapse everything instantly
        ...
        // Resolve active displacement mode: section override > user param
        const activeDisplaceMode = this._sectionDisplaceMode || (params.displaceMode ?? 'frequency');

        // ─── GUN SHOT → instant explode spike ───
        if (audio.gunShotDetected) {
            this.explodePhase = Math.max(this.explodePhase, (audio.gunShotIntensity || 1) * (params.beatExplode || 1) * 1.5);
        }
        if ((audio.gunShotDecay || 0) > 0) {
        ...
        const sub    = audio.smoothBands.sub;
        ...
        // Wobble LFO → displacement breathes at wobble rate
        if (audio.hasSustainedBass && (audio.wobbleIntensity || 0) > 0.2) {
            const wobbleMod = 0.5 + 0.5 * (audio.wobbleLFO || 0);
            dAmt *= (0.4 + 0.6 * wobbleMod);
        }

        // Beat explode — driven by beatPhase for rhythmic snap
        if (audio.bassBeat && params.beatExplode > 0) {
            this.explodePhase += audio.bassBeatIntensity * params.beatExplode * 0.12;
        }
        this.explodePhase = Math.min(this.explodePhase, 1.5);
        this.explodePhase *= 0.92;

        // Bass breathing locked to beatPhase
        ...
        const chromatic = (params.chromaticSplit ?? 0) + (audio.screechIntensity || 0) * 3.0;

        // Siren rising → Y-axis stretch (computed once, used in vertex loop)
        const sirenStretchAmt = (audio.sirenRising || 0) > 0.3
            ? (audio.sirenRising || 0) * dAmt * 0.5 * (audio.sirenRising || 0)
            : 0;
        ...
        for (let i = 0; i < vertCount; i++) {
            ...
            // Morphing — smooth hermite blend to target
            ...
            const chromaticOffset = chromatic > 0 ? chromatic * freq : 0;

            solidPos[i3]     = (bx + nx * disp * patMod) * scale + (chromaticOffset > 0 ? Math.sin(t * 6.28 + this.time) * chromaticOffset : 0);
            solidPos[i3 + 1] = (by + ny * disp * patMod) * scale - gravAmt * t * 0.5 + ny * sirenStretchAmt;
            solidPos[i3 + 2] = (bz + nz * disp * patMod) * scale + (chromaticOffset > 0 ? Math.cos(t * 6.28 + this.time) * chromaticOffset : 0);
        ...
        const rx = (params.rotSpeedX ?? 0.3) * (1 + mid    * react);
        const ry = (params.rotSpeedY ?? 0.5) * (1 + bass   * react);
        const rz = (params.rotSpeedZ ?? 0.1) * (1 + treble * react);
        ...
        // Wrap to prevent float32 precision loss over long sessions
        this._rotWrapCounter++;
        if (this._rotWrapCounter > 600) {
            this._rotWrapCounter = 0;
            const PI2 = Math.PI * 2;
            this.group.rotation.x %= PI2;
            this.group.rotation.y %= PI2;
            this.group.rotation.z %= PI2;
        }

        // Beat spin burst — beat-phase-locked for tight grid feel
        if (audio.bassBeat) {
            ...
            const rotBoost = 1 + bpulse * burst * _speedScale;
            this.group.rotation.y += Math.min(0.04, audio.bassBeatIntensity * 0.08 * burst * rotBoost);
        }

        // Fakeout: dampen all rotation axes until section ends
        if (this._fakeoutLock) {
            this.group.rotation.x *= 0.95;
            this.group.rotation.z *= 0.95;
        }

        // Drop decay: subtle Z tilt
        ...
        // ─── PARTICLES ───
        if (params.emitParticles) {
            this.emitParticlesFromMesh(audio, params);
    ...
    updateGhosts(params) {
        ...
        while (this.ghosts.length > count) {
            ...
            this.group.remove(g); g.geometry.dispose(); g.material.dispose();
        }
    ...
    emitParticlesFromMesh(audio, params) {
        ...
        for (let e = 0; e < emitCount; e++) {
            ...
            this.particlePositions[idx * 3]     = vec.x;
            this.particlePositions[idx * 3 + 1] = vec.y;
            this.particlePositions[idx * 3 + 2] = vec.z;
    ...
    updateMirrors(params, audio) {
        ...
        while (this.mirrorMeshes.length < count) {
            ...
            this.mirrorMeshes.push(mesh);
        }

        // Declare solidPos ONCE outside the loop
        const solidPos = this.meshSolid.geometry.attributes.position.array;

        for (let i = 0; i < count; i++) {
            const m     = this.mirrorMeshes[i];
            const angle = ((i + 1) / (count + 1)) * Math.PI * 2;
            m.visible     = true;
            m.position.x  = Math.cos(angle + this.time * 0.2) * dist;
            m.position.y  = Math.sin(angle * 0.5 + this.time * 0.1) * dist * 0.3;
            m.position.z  = Math.sin(angle + this.time * 0.2) * dist;
            m.rotation.copy(this.group.rotation);
            if (i % 2 === 0) m.scale.x = -1;
            else m.scale.setScalar(0.7 + audio.smoothBands.treble * 0.5);
            m.material.color.setHSL((this.time * 0.05 + i * 0.12) % 1, 0.9, 0.5);
            m.material.opacity = 0.15 + audio.rms * 0.2;
            // Sync deformed shape to mirror
    ...
    destroy(scene) {
        if (this.group) {
            this.group.traverse(c => {
                ...
                if (c.material) {
                    ['map', 'normalMap', 'emissiveMap', 'envMap'].forEach(slot => { if (c.material[slot]) c.material[slot].dispose(); });
                    c.material.dispose();
                }
            });
        ...
        this.basePositions = null; this.normals = null;
        this.ghosts = []; this.particleSystem = null;
        this.morphTargetBase = null; this.morphing = false;
        this._sectionDisplaceMode = null; this._fakeoutLock = false;
    }
};